...
class fastTextB:
    ...
    def loss(self,l2_lambda=0.01): #0.0001-->0.001
        ...
        if self.is_training: #training
            ...
            loss = tf.nn.softmax(logits=self.logits)
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for i in range(1000):
            ...
            input_x = np.random.uniform(0, 1, size=(batch_size,sequence_length))
            # input_y = np.array([1,0,1,1,1,2,1,1],dtype=np.int32) #np.zeros((batch_size),dtype=np.int32) #[None, self.sequence_length]
            input_y = np.array([1, 1, 3, 3, 3, 2, 2, 1], dtype=np.int32)  # np.zeros((batch_size),dtype=np.int32) #[None, self.sequence_length]
            print(u"initialization finished")
